...
- Greeting and Log-in area
    ...
    - Test: I tested this feature on my son. He likes this part a lot because he can type everything he wants and then read these funny words which he sees on the game area and modal window areas later on. It develops imagination. After two days he can type his name easily because he remembers where the letters are placed on the keyboard.
...
- Question area and Answer buttons
   - This is where the easy addition is displayed when the start button is clicked. The two random numbers are selected by nextQuestion function.
   - Two green round buttons change the color to light yellow when hovered. This feature provides the user with good navigation and enables him to react fast to the question.
 ...
 - Rules Modal window
    - The rules modal window consists of the user's name saved from the log-in area, the presentation of rules, audio icon with recorded rules. I recorded myself as if Bob speaks to the user (child).
    - If the audio off icon is clicked it turns to the on icon. For a better user experience the audio does not autoplay until the user decides so. To achieve it the Java Script Boolean was used.
...
- The Design for this website was inspired by my older son, who likes crabs and math. He is almost 4 and wants to learn to add numbers not only with the fingers. That is why I decided to make a game for him.
- This inspired me to make an interactive game where crab speaks and plays with a kid. The crab asks what the child's name is and reads rules for it. 
- My son speaks two languages and likes to play with the words. In this game he can learn to type his name and pick a new one for fun.

- The Irish Grover font I found in one article about game fonts. I used it for the game title. The Source Sans 3 was chosen for the text in the game. Both fonts are from Google Fonts.
...
- The audio button works when it is clicked. The on icon switches to the off icon and plays the audio. And the off icon switches to the on icon when clicked respectively.

- I tested the functionality of the game with my son. 
He likes emoji very much and sometimes answers wrong to see the crying emoji. Collecting scores has not a big meaning. Nevertheless he he likes to know how many correct answers he did.

- He has a lot of fun typing his name in the log-in area. He tested that he cannot type more than 10 letters and that the first letter is capitalized. He asked why is that. Then I showed him the text from the error message.

- The modal window opens when the rule icon is clicked. 

- The audio plays only when we click on it and stops when we don't want to listen anymore.

- It was found out that when the audio ware turned off and then clicked again to play it continue from the second it was stopped. That is because the function is "pause".

### **Bugs**

- Problem 1:  When adding a timer to start the game the timer interval didn't appear on the screen. Only the hourglass icon war there.
  - *Solution 1: span element was added to the interval.*

- Problem 2: Beim clicking "submit-btn" without user verification, the button led to the game screen.
...
- Problem 3:  The number of scores in the new game was added to the score number from the previous game.
  - *Solution 3: const currentScore = document.getElementById("current-score"); was changed to global variable let. Changed the value of the score variable to 0 because it carried the score value from the previous game*

- Problem 4:  The overlay for the modal window was placed at the right corner of the bottom of the website.
...
- Find this link [here](https://zhannamatuzak.github.io/math-with-crab/)

## Credits 
------
### **Code**

- [Juliia Konovalova](https://github.com/IuliiaKonovalova) explained to me the ways I can shorten my code for a better navigation experience.
...
- To start a new game the resetTimer function was needed. I looked into the project code of [Alfred](https://github.com/AlfredA93), a student from my cohort at Code Institute. His mentor helped him to understand the syntax of the timer function. Alfred wrote a simple version of it, which I adapted to my code. 

  ![timer credit](/documentation/reset-timer.png)

- Also, I watched the video tutorial from [Mr.Soderquist](https://www.youtube.com/channel/UCoL-30L7NMRNzwiHUqSYCVQ), which assisted me in how to set the timer and check the answer and set the high score using Window localStorage Property.

- The log in feature from the [project](https://aleksandracodes.github.io/CI_PP2_SunshineGuessing/index.html) of Aleksandra H. has inspired me. And I used it as a communication method between the crab, the game owner, and the user, who is a child. The crab wants to learn more about the child so he asks him to write the name. The name is used then in the game field and the rules modal window. 
